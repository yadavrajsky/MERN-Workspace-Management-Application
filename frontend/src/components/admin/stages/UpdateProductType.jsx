...
const UpdateProductType = () => {
  const { id } = useParams();
  const { selectedCoworkingSpace, error, message, loading } = useSelector(
    (state) => state.coworkingSpaces
  );

  const [propertyName, setPropertyName] = useState(selectedCoworkingSpace?.name|"");
  const [productType, setProductType] = useState(selectedCoworkingSpace?.setProductType | "");
  const [city, setCity] = useState(selectedCoworkingSpace?.location.city | "");
  const [locality, setLocality] = useState(selectedCoworkingSpace?.location?.locality |"");
  const [brand, setBrand] = useState(selectedCoworkingSpace?.brand | null);
  const [creditsPerDeskPerMonth, setCreditsPerDeskPerMonth] = useState(selectedCoworkingSpace?.creditsPerDeskPerMonth| 0);
  const [supportsParking, setSupportsParking] = useState(selectedCoworkingSpace?.supportsParking | false);
  const [supportsMetroConnectivity, setSupportsMetroConnectivity] =
    useState(selectedCoworkingSpace?.supportsMetroConnectivity | false);
  const [selectedAmenities, setSelectedAmenities] = useState(selectedCoworkingSpace?.amenities|[]);
  ...
  useEffect(() => {
    if (draftCoworkingSpace) {
      ...
      setBrand(draftCoworkingSpace.brand || "");
